...
export class FirestoreUserRepository implements IUserRepository {
  ...
  async findByIds(userIds: string[]): Promise<Array<Partial<User> & { id: string; exists: boolean }>> {
    ...
    return docs.map(doc => ({
      ...
      exists: doc.exists,
      ...(doc.exists ? doc.data() : {})
    })) as Array<Partial<User> & { id: string; exists: boolean }>;
  }
  ...
  async findAll(): Promise<User[]> {
    ...
    return snapshot.docs.map((doc: admin.firestore.QueryDocumentSnapshot) => ({ id: doc.id, ...doc.data() } as unknown as User));
  }
